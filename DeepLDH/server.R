...
shinyServer(function(input, output) {
  ...
  observe({
    ...
    library(tfruns)
    FLAGS <- tfruns::flags(
      flag_numeric('dropout1', 0.3, 'drop1'),
      flag_numeric('dropout2', 0.3, "drop2"),
      flag_numeric("dropout3", 0.2, "drop3"),
      flag_numeric("dropout4", 0.2, "drop4"),
      flag_integer('u1', 256, 'Units1'),
      flag_integer("u2", 256, "units2"),
      flag_integer("u3", 64, "units3"),
      flag_integer("u4", 32, "units4"),
      flag_integer("u5", 4, "units5"),
      flag_integer("batch", 15, "batchsize"),
      flag_integer("epoch", 100, "epochs"),
      flag_numeric("alpha", 0.002, "learningrate")
    )
    ...
    model_keras2 %>% load_model_weights_hdf5("back.h5")
    ...
    library(tfruns)
    FLAGS <- tfruns::flags(
      flag_numeric("dropout0", 0.5, "drop0"),
      flag_numeric('dropout1', 0.4, 'drop1'),
      flag_numeric('dropout2', 0.3, "drop2"),
      flag_numeric("dropout3", 0.2, "drop3"),
      flag_numeric("dropout4", 0.1, "drop4"),
      flag_numeric("dropout5", 0.1, "drop5"),
      flag_integer("u0", 256, "unit0"),
      flag_integer('u1', 256, 'Units1'),
      flag_integer("u2", 128, "units2"),
      flag_integer("u3", 64, "units3"),
      flag_integer("u4", 32, "units4"),
      flag_integer("u5", 4, "units5"),
      flag_integer("batch", 10, "batchsize"),
      flag_integer("epoch", 250, "epochs"),
      flag_numeric("alpha", 0.0005, "learningrate")
    )
    ...
    model_keras3 %>% 
      layer_dense(units = FLAGS$u0, activation = "relu", input_shape = c(21)) %>%
      #layer_batch_normalization() %>%
      layer_dropout(rate = FLAGS$dropout0) %>%
      layer_dense(units = FLAGS$u1, activation = "relu") %>%
      layer_dropout(rate = FLAGS$dropout1) %>%
      layer_dense(units = FLAGS$u2, activation = "relu") %>%
      #layer_batch_normalization() %>%
      layer_dropout(rate = FLAGS$dropout2) %>%
    ...
    model_keras3 %>% load_model_weights_hdf5("odi.h5")
    
    observeEvent(input$predict, {
      probodi <- model_keras3 %>% predict_proba(dfl)
      predodi <- model_keras3 %>% predict_classes(dfl) %>% as.vector()
      probodi <- probodi[1,2]
